...
if __name__ == '__main__':
    ...
    excluded_subjects, \
        ...
        balance_classes, \
        target_col_name,\
        kind = vars(parser.parse_args()).values()
    ...
    for subject_name in iterator:
        ...
        if 'sp' in kind:
                ...
                preprocessed_Y.append(np.zeros((len(preprcessed[-1].clusters),)))
                preprcessed.append(sp_preprocessor(iterator.get_data(STAGE.POSTTEST)))
                preprocessed_Y.append(np.ones((len(preprcessed[-1].clusters),)))
        if 'con' in kind:
        ...
        Y = np.concatenate(preprocessed_Y)
