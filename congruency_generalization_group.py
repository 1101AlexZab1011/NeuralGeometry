...
if __name__ == '__main__':
    ...
    for subject_name in iterator:
        if not os.path.exists(iterator.dataset_path[:-3] + '_train.pt') or info is None:
            ...
            Y_train = np.concatenate([data.session_info[target_col_name].to_numpy() for data in preprcessed_training])
            ...
            Y_test = np.concatenate([data.session_info[target_col_name].to_numpy() for data in preprcessed_testing])
