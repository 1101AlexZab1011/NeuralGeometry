...
import torchmetrics
from utils import PenalizedEarlyStopping, TempConvAveClipping, IndependanceConstraint, accuracy
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--targets', type=str, nargs='+',
                        help='Targets to predict (must be a column from sesinfo csv file with either 0 or 1 values)',
                        default=['CommonCoords', 'SpecificNew', 'SpecificOld', 'center', 'unique']
                        )
    ...
    for subject_name in iterator:
        ...
        Y = np.concatenate([data.session_info[target_col_names].to_numpy() for data in preprcessed])
        Y = one_hot_decoder(Y)
