...
if __name__ == '__main__':
    ...
    for subject_name in iterator:
        if not os.path.exists(iterator.dataset_path) or info is None:
            ...
            dataset_test = EpochsDataset((X_train, Y_train), transform=zscore, savepath=iterator.dataset_content_path + '_test')
            ...
            all_train_data.append(EpochsDataset.load(iterator.dataset_path[:-4] + '_train.pt'))
            all_test_data.append(EpochsDataset.load(iterator.dataset_path[:-4] + '_test.pt'))
