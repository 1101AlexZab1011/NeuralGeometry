...
if __name__ == '__main__':
    ...
    for subject_name in iterator:
        if not os.path.exists(iterator.dataset_path) or info is None:
            ...
            masks = [data.session_info.priming == 0 for data in preprcessed]
            logging.debug(f'for all stages overall data size: {[len(data.session_info) for data in preprcessed]}')
            logging.debug(f'for all stages masked data size: {[len(np.where(mask.to_numpy()==True)[0]) for mask in masks]}')
            X = np.concatenate([
                data.
                epochs.
                pick_types(meg='grad').
                ...
                get_data()[mask]
                for data, mask in zip(preprcessed, masks)
                ])
            logging.debug(f'Final number of samples: {len(X)}')

            if subject_num%2:
            # if subject_num < len(os.listdir(subjects_dir))//2:
            # if subject_num in a:
                Y = np.ones((len(X), 1))
            else:
                Y = np.zeros((len(X), 1))
            ...
            logging.debug(f'Dataset for {subject_num%2}s is made (subject {subject_num}), {len(X)} in total')
        else:
